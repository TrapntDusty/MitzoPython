...
class Help(commands.Cog, name="Comando Ayuda"):
    ...
    async def setup_help_pag(self, ctx, entity=None, title=None):
        ...
        for i in range(0, len(filtered_commands), self.cmds_per_page):
            ...
            for cmd in next_commands:
                ...
                subcommand = "tiene SubComandos" if hasattr(cmd, "all_commands") else ""
    ...
    @commands.command(
        name="help", aliases=["h", "comandos"], description="el comando ayuda!"
    )
    async def help_command(self, ctx, *, entity=None):
        if not entity:
            ...
            if cog:
                await self.setup_help_pag(ctx, cog, f"{cog.qualified_name}'s comandos")
                ...
                if command:
                    ...
                    await ctx.send("entidad no encontrada.")
